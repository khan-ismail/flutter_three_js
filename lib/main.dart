...
void main() {
  runApp(const Simple3DViewerApp());
}

class Simple3DViewerApp extends StatelessWidget {
  const Simple3DViewerApp({super.key});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      home: const Simple3DViewer(),
    );
  }
}

class Simple3DViewer extends StatefulWidget {
  const Simple3DViewer({super.key});

  @override
  _Simple3DViewerState createState() => _Simple3DViewerState();
}
...
class _Simple3DViewerState extends State<Simple3DViewer> {
  ...
  Future<void> setup() async {
    // Set up camera
    threeJs.camera = three.PerspectiveCamera(85, threeJs.width / threeJs.height, 1, 1000);
    ...
    pointLight.position.setValues(5, 10, 7);
    threeJs.scene.add(pointLight);

    // Load 3D model
    final loader = three.GLTFLoader(flipY: true).setPath('assets/');
    final model = await loader.fromAsset('city_rtx.glb'); // Replace 'your_model.glb' with your file name
    if (model != null) {
      threeJs.scene.add(model.scene);
    }
